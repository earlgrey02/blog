...
tags: [Kotlin, Spring]
---
이번 글은 멀티 모듈 기반 헥사고날 아키텍처를 구성한 경험에 대한 글입니다.
...
앞서 설명드렸듯이, 도메인 주도 설계(Domain Driven Design, DDD)를 따르는 헥사고날 아키텍처에서는 비즈니스 로직이 외부 기술에 영향받지 않아야 합니다.
하지만 Spring WebFlux를 기반으로 서버를 개발한다면 이러한 비동기 프로그래밍이 필수적으로 요구되는 상황이었습니다.
...
그래서 Reactor보다는 명령형 프로그래밍 방식으로 비동기 처리가 가능한 코루틴이 비즈니스 로직의 가독성에 덜 영향을 주지 않을까 생각해서 코루틴을 도입하기로 했습니다.
또한 코루틴은 기존의 명령형 프로그래밍을 사용하므로 Spring WebFlux에서 Spring Web MVC로 변경해도 비즈니스 로직에 큰 변화를 주지 않습니다.
...
비즈니스 로직을 작성하는 중에 저는 `CreateUserPort`에서 `createUser`를 호출하는 부분이 불필요하다고 생각헀습니다.
포트는 단일 책임 원칙에 의해 하나의 책임만 가지며, 해당 책임에 대한 정보는 이미 포트에 이름에 명시되어 있는데요.
그럼 굳이 메서드를 호출해서 중복되는 의미를 만들 필요가 없다고 생각했습니다.
그래서 이 부분은 연산자 오버로딩을 통해 해결했습니다.

```kotlin title="CreateUserPort.kt" showLineNumber {2}
...
확실히 이전 코드와 달리 불필요하게 중복되는 의미도 없어지고 코드도 간결해지는 것을 확인할 수 있습니다.
