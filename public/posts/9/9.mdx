...
그리고 `jenkins-values.yaml`에서 설정하고자 하는 부분을 수정하면 됩니다.
저는 Ingress를 Root 권한으로 실행하고 이미 존재하는 `PersistenceVolumeClaim`을 사용하도록 했습니다.
또한 `/jenkins`를 Prefix로 가지는 Ingress를 생성할 것이므로, 이에 맞게 `jenkinsUriPrefix`도 수정했습니다.
...
일반적으로 GitOps에서 Helm 차트는 Git을 통해 관리하게 됩니다.
