...
특히 여러 테스트를 계층화할 때 `@Nested`와 함께 많은 내부 클래스를 사용하다 보니 코드가 복잡해지기 쉬웠습니다.
...
Kotest는 Kotlin 기반 테스트 프레임워크입니다.
...
Kotest와 함께 자주 사용되는 Mockk는 Kotest처럼 Kotlin DSL을 통해 모의 객체를 설정하고 검증하는 모킹(Mocking) 프레임워크입니다.
...
이후 모든 컨트롤러 테스트 클래스들은 `ControllerTest`를 상속받기만 하면 됩니다.
...
Spring WebFlux에서는 서비스 계층에서 `Mono` 또는 `Flux` 같은 `Publisher`를 반환하므로 구독을 통해 검증을 해야 합니다.
...
참고로 Kotest는 코루틴 기반으로 작동하므로 추가적인 코루틴 빌더 없이 코루틴 확장 함수를 사용할 수 있습니다.
