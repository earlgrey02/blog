...
---
이번 글은 Spring WebFlux에서의 로깅(Logging) 및 로그 추적에 대한 글입니다.
...
하지만 위와 같이 라우터에서의 핸들러 매핑 등의 정보도 함께 로그에 출력되는데 이러한 내용들은 로그로 수집할 필요가 없는 내용들이라고 생각했습니다.
...
제가 구현할 로깅은 부가적인 기능 중 하나이므로 AOP의 모듈화 대상이 되는데요.
...
앞서, 현재 코드는 Spring WebFlux의 함수형 엔드포인트 방식을 기반으로 작성되어 있습니다.
...
실제 요청과 응답은 `HandlerFunction<ServerResponse>`의 인자와 반환 값인 `ServerRequest`와 `Mono<ServerResponse>`가 됩니다.
...
그렇게 출력된 로그를 확인하는데 API가 정상적으로 요청을 처리하지 못하는 문제가 발생했었습니다.

## Mono의 한계

문제의 원인은 어드바이스(Advice)에서 `Mono`로부터 Request Body를 가져오는 과정에 있었습니다.
...
여기서 `bodyToMono()`는 단순히 Request Body를 반환하는 것이 아닌 Request Body를 발행하는 `Publisher`의 구현체인 `Mono`를 반환하게 됩니다.
`Mono`는 단일 요소 스트림이므로 한번 구독을 통해 값을 발행받으면 그 이후에는 더 이상 값을 받을 수 없습니다.
...
앞서, `LoggingAspect`에서 `bodyToMono()`를 통해 얻은 `Mono`를 구독해 값을 발행받았으니, 이후 핸들러 계층에서 호출되는 `bodyToMono()`로 얻은 `Mono`는 더 이상 값을 발행하지 않습니다.
...
당연히 `bodyToMono()`가 반환하는 `Mono`도 새로운 `Mono`이므로 핸들러 계층에서 Request Body를 다시 읽을 수 있게 됩니다.
...
`body()`는 JSON 문자열을 통해 새로운 Request Body를 설정합니다.
...
앞서 설명했듯이, `bodyToMono()`로부터 가져온 Request Body와 기존의 `ServerRequest`를 통해 새로운 `ServerRequest`를 생성하도록 했습니다.
...
그 결과, 정상적으로 요청이 처리되어 로그가 출력되는 것을 확인 할 수 있습니다.
...
하지만 예외가 발생하는 경우, 로그에 해당 예외에 대한 상태 코드가 반영되지 못하는 문제가 발생했었습니다.
...
저는 모든 핸들러에서 `ServerResponse.ok()`를 통해 `ServerResponse`가 `200 OK`의 상태를 가지도록 구현해왔는데요.
...
특히 Request Body가 비어있는 경우, `join()`이 `Mono.empty()`를 반환하므로 `Mono.empty()`로 인해 다운 스트림이 전부 실행되지 않는 문제가 발생할 수 있는데요.
그래서 `defaultIfEmpty()`를 사용해 크기가 0인 `ByteArray`를 Request Body의 기본 값으로 설정해 주었습니다.

<br />

또한 버퍼(Buffer)의 경우, 사용한 후에 남은 레퍼런스 카운트(Reference Count)에 의해 자동으로 가비지 컬렉션(Garbage Collection)의 대상이 되지 않는데요.
...
이전과 달리 `GlobalExceptionHandler`에 의해 변환된 응답이 로그에 반영된 것을 확인할 수 있었습니다.
...
이렇게 식별자를 이용한 로그 추적에는 MDC(Mapped Diagnostic Context)를 활용할 수 있습니다.
...
당연히 Slf4j 구현체를 사용하는 Spring에서도 MDC를 사용할 수 있습니다.
...
이로 인해 스레드에 의존적인 `ThreadLocal`을 사용하기는 어려우므로 MDC를 통한 로그 추적도 어렵습니다.
