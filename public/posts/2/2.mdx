...
---
이번 글은 Spring Batch와 Kubernetes의 CronJob을 통해 구현한 배치 작업에 대한 글입니다.
...
이렇게 되면 제가 어떤 성능 최적화를 수행하든 학교 API에 의존하는 이상 최적화의 의미가 없어진다고 생각했습니다.
그래서 주기적으로 학교 API로부터 데이터를 가져와 데이터베이스에 저장하기로 했습니다.
이러면 기존의 학교 API를 사용하는 부분에서 지연 시간이 짧은 데이터베이스를 사용할 수 있게 됩니다.
...
배치 작업을 구현하는데에는 Spring Batch와 Kubernetes의 CronJob을 사용하기로 했습니다.
...
그러므로 따로 존재하는 차트 레포지토리 내에 CronJob에 대한 매니페스트(Manifest)를 작성합니다.
