...
이는 동기와 다르게, 응답이 오기까지 대기할 필요가 없다는 의미이기도 합니다.
...
이 이유는 `setTimeout()`이 비동기적으로 수행되기 때문에 인터프리터가 결과를 기다리지 않고 다음 줄을 실행하기 때문입니다.
...
`readFileSync()`가 수행되는 동안 해당 스레드는 다른 작업을 할 수 없으며, 작업이 끝나고 나서야 다음 줄이 실행됩니다.
...
결과를 보면 실제로 동기적으로 작업이 처리되는 것을 확인할 수 있으며, `await`으로 인해 함수가 일시적으로 중지되어도 해당 스레드는 계속해서 작업을 수행할 수 있으므로 논블로킹입니다.
...
`await`을 사용했음에도 스레드가 다른 작업을 수행할 수 있는 이유는 `await`이 내부적으로 `await` 아래의 코드들이 위 코드처럼 프로미스 체인 내부로 들어가는 것과 유사하게 작동하기 때문입니다.

## 비동기 & 블로킹

비동기 및 블로킹 방식은 요청과 응답이 동시에 처리되지 않음에도 그 사이에 스레드가 다른 작업을 수행할 수 없습니다.
사실상 비동기의 장점이 없어지며 사용할 이유가 없는 방식이기도 합니다.
...
`setTimeout()`의 호출과 종료가 동시에 처리되지도 않으며, 그 사이에 스레드가 다른 작업도 수행할 수 있음을 확인할 수 있습니다.
