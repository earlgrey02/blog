...
저는 백엔드 개발자라면 벗어날 수 없는 API 문서화를 편리하게 할 수 있는 방법을 모색하고 있었습니다.
...
이 점에서 프로덕션 코드에 문서화 관련 코드가 포함되는 것이 싫었던 저는 Swagger가 아닌 Spring REST Docs를 선택했었습니다.

# Spring REST Docs with Swagger

결국 이렇게 상반된 장단점을 가진 Spring REST Docs와 Swagger를 함께 사용해보기로 했습니다.
우선 Spring REST Docs가 생성한 스니펫들을 Swagger가 읽을 수 있도록 OpenAPI 명세에 맞춘 문서로 변환하고, 해당 문서를 Swagger로 읽으면 됩니다.
