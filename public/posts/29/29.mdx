---
title: Kqueue로 구현해보는 I/O Multiplexing
...
그래서 불가피하게 BSD(Berkeley Software Distribution) 계열 운영체제의 `epoll()`인 `kqueue()`를 통해 이벤트 루프를 구현해 보겠습니다.
...
```cpp showLineNumbers {1, 6-7, 38-40, 24-25}
...
특히 Node.js는 공식 문서에 ['Don't Block the Event Loop'](https://nodejs.org/en/learn/asynchronous-work/dont-block-the-event-loop)라는 제목의 페이지가 존재할 정도입니다.
