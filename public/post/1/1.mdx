---
title: Spring WebFlux에서의 로깅(Logging)
...
하지만 위와 같이 핸들러 매핑 등의 불필요한 정보도 로그로 출력되는데요.
...
하지만 이를 해결하고자 핸들러 계층에서 예외 처리를 수행하도록 하는 것은 핸들러 계층의 책임이 너무 커지고 중복 코드가 발생하게 된다고 생각했습니다.
...
제가 구현한 로그 시스템에서 아쉬운 점이 하나 있었습니다.
