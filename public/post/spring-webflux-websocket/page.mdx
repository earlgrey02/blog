...
description: Spring WebFlux와 Kafka로 채팅 기능 구현하기
date: 2024-06-14
tags: [Kotlin, Spring, Apache Kafka]
---
...
![](/post/spring-webflux-websocket/1.png)

실시간 통신을 구현하는 방법으로는 폴링(Polling), SSE(Server-Sent Events), 웹 소켓(Web Socket)이 있는데요.
...
사실 메시지를 보내는 것은 단순히 API를 통해서, 메시지를 받는 것은 웹 소켓 방식처럼 받으면 SSE만으로도 채팅 기능을 구현할 수 있습니다.
...
![](/post/spring-webflux-websocket/2.png)
...
![](/post/spring-webflux-websocket/3.png)
![](/post/spring-webflux-websocket/4.png)
...
웹 소켓은 서버에 종속적이라 서버 인스턴스가 분산되어 있는 분산 시스템에서는 메시지를 받을 사용자가 다른 서버의 세션으로 존재할 수 있습니다.
이 경우, 메시지가 정상적으로 수신되지 않는 문제가 발생하는데요.
...
![](/post/spring-webflux-websocket/5.png)
![](/post/spring-webflux-websocket/6.png)
...
![](/post/spring-webflux-websocket/9.png)
![](/post/spring-webflux-websocket/10.png)
...
![](/post/spring-webflux-websocket/11.png)

또한 다른 케이스에서는 아예 상대방에게 메시지가 전달이 안 되는 문제를 확인했는데요.
...
![](/post/spring-webflux-websocket/12.png)
...
![](/post/spring-webflux-websocket/13.png)
![](/post/spring-webflux-websocket/14.png)
