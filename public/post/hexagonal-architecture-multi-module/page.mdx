...
그래서 저는 어떤 기술을 사용할지보다는 중간에 다른 기술로 바꾼 경우를 좀 더 고려해 보기로 했습니다.
...
![](/post/hexagonal-architecture-multi-module/1.png)
...
![](/post/hexagonal-architecture-multi-module/2.png)
...
![](/post/hexagonal-architecture-multi-module/3.png)
...
문제는 Reactor의 여러 리액티브 연산자들로 인해 비즈니스 로직의 가독성이 안 좋아지는 것을 경험해 왔는데요.
...
![](/post/hexagonal-architecture-multi-module/4.png)
...
비즈니스 로직을 작성하는 도중, 저는 `CreateUserPort`에서 `createUser()`를 호출하는 부분이 의미상 중복이라고 생각했습니다.
...
![](/post/hexagonal-architecture-multi-module/5.png)
